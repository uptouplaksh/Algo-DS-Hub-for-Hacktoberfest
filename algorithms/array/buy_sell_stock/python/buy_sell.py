...
def max_profit(prices: List[int]) -> int:
    ...
    minimum price seen so far and the maximum profit that can be achieved.

    Complexity Analysis:
    - Time Complexity: O(n), where n is the number of days (the length of the
      prices list). We go through the list in one run.
    - Space Complexity: O(1), as we only use a constant amount of extra
      space to create the variables min_price, and max_profit.
    ...
    for price in prices:
        ...
        if potential_profit > max_profit_val:
            # If this potential profit is greater than our recorded max profit, update the value
            max_profit_val = potential_profit
    return max_profit_val


# Runnable Example Block (extracted from leetcode)
